...
DATA "ÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜ", 7,-1,5
...
SUB DoExplosion (x#, y#)
 ...
 ELSE
  ...
  'Ã¾ Draw grey smoke, EGA/VGA only
  IF Mode = 9 THEN
...
FUNCTION DoShot (Player$(), PlayerNum, x, y, turn, othX, othY)
  ...
  ELSE
   ...
   IF PlayerHit <> PlayerNum AND turn < 3 THEN
    'Ã¾ Killed opponent in one shot message
...
FUNCTION Get$ (Row, Col, Prev$, Typ, Max, Esc)
 ...
 DO
   DO
     Timo! = TIMER: Curs = 0: LOCATE Row, Col + LEN(Hold$): IF LEN(Hold$) = Max THEN PRINT "Ã›" ELSE PRINT "_"
     DO: LOOP UNTIL INKEY$ = ""
     DO
       IF TIMER > Timo! + .5 THEN
         ...
         IF Curs <> 1 THEN
          PRINT " "
         ELSE
          IF Typ AND LEN(Hold$) = Max THEN PRINT "Ã›" ELSE PRINT "_"
         END IF
         Curs = 1 - Curs
         Timo! = TIMER
       END IF
   ...
   SELECT CASE Intra
     ...
     CASE 1 TO LEN(Valid$) - 4 'Ã¾ Letter, number or symbol
     'Ã¾ Numeric field
      IF NOT Typ THEN 'Ã¾ Number
...
' Lay out screen
 ...
 CLS : RESTORE Setup: Slidy: COLOR 2: LOCATE 2, 1: PRINT STRING$(80, "Ä") 'Ã¾ Show screen title
 active = 0: FOR fld = 1 TO 4: GOSUB SetupFields: NEXT 'Ã¾ Display fields
 fld = 0: GOSUB SetupFields 'Ã¾ Display player names

' Fill in players box

 cStat = 0: FOR N = 1 TO P: GOSUB Curs: NEXT
 'Ã¾ Must highlight opponent player (normally done after [ENTER] or [TAB]
...
' Process fields loop
 ...
 DO
  'Ã¾ Highlight current field if there are enough players. Player field not
  ...
  SELECT CASE fld
   ...
   CASE IS = 4
    GOSUB Gravity
  END SELECT
  active = 0: GOSUB SetupFields 'Ã¾ Unhighlight current field
...
'Ã¾ Clear most of the screen
COLOR , 0: FOR l = 3 TO 24: LOCATE l, 1: PRINT STRING$(80, " "); : NEXT

EXIT SUB

'Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾Ã¾
'Ã¾ FIELDS SUBROUTINES

ManagePlayers:
 cre = 0
 WHILE P < 2 'Ã¾ Ensure enough players for the game (only used before league table created)
  cre = 1: GOSUB CreatePlayer
  IF P = 2 THEN GOSUB SetupFields 'Ã¾ Finally ready to highlight Player field
 WEND
 cre = 0

 'Ã¾ OK. Assuming that there are enough players to select.
 opp = 2 / fld 'Ã¾ PDefs array number of opposite player
 ShowPrompts fld
 IF PDefs(fld) > 0 THEN 'Ã¾ Put cursor bar on currently selected player
  x = ((PDefs(fld) - 1) MOD 4) + 1: y = INT((PDefs(fld) - 1) / 4) + 1
 ELSE                   'Ã¾ Otherwise choose free player
  IF PDefs(opp) <> 1 THEN
   x = 1: y = 1
  ELSE
 ...
 DO
 defSwap = 0 'Ã¾ Flag for player definition swapping
  N = (y - 1) * 4 + x 'Ã¾ Convert cursor bar position into player number
  LOCATE 8 + (fld * 2 - 2), 22
  'Ã¾ Do not display player name if it is taken and swap is not permitted
  ...
  'Ã¾ Move cursor bar, manipulate players, and select a player to compete
  SELECT CASE UCASE$(key$)
   ...
   CASE CHR$(0) + CHR$(77)
    ...
    END IF
   CASE CHR$(9), CHR$(13)
    IF key$ = CHR$(13) THEN 'Ã¾ Only update player defs if ENTER pressed
     IF PDefs(opp) = N AND PDefs(fld) > 0 THEN
      'Ã¾ Swap player definitions
      SWAP PDefs(1), PDefs(2): COLOR , 0: finished = 1
      cStat = 2: GOSUB Curs
      IF fld = 2 THEN N = PDefs(opp): GOSUB Curs
     ELSEIF PDefs(opp) <> N THEN
      'Ã¾ Define player
      IF PDefs(fld) <> N THEN 'Ã¾ Remove green highlight and define PDefs
    ...
    ELSE
     IF PDefs(fld) > 0 THEN
      'Ã¾ Abort change to definition, and move to next field
      finished = 1
      cur = 0: GOSUB Move 'Ã¾ Remove cursor bar
      N = PDefs(fld): cStat = 2: GOSUB Curs 'Ã¾ Red highlight
     ELSEIF PDefs(fld) = 0 AND PDefs(opp) <> N THEN
      'Ã¾ Player undefined, so define it
...
CreatePlayer:
 IF P < NPLAYERS THEN
  ...
  PDat(P + 1).PNam = " "
  DO: cont = 1
   IF P < 2 THEN Esc = FALSE ELSE Esc = TRUE 'Ã¾ Prevent ESCAPE key when players not yet created
...
DeletePlayer:
 'Ã¾ What to do after the delete
 nextAction = 0
 IF N = PDefs(opp) THEN 'Ã¾ Opposite player redefined
  IF NOT (fld = 1 AND P > 2) THEN 'Ã¾ But not in this situation
   nextAction = 1
  END IF
 END IF
 ...
 IF i$ = "Y" THEN
  'Ã¾ Corrects PDefs (selected players) values and display
  ...
  'Ã¾ Tidies up PDat (array of players)
  Pt = P: P = P - 1: Nt = N
  IF N < Pt THEN
   FOR N = N TO P
  ...
  'Ã¾ This wipes all trace of the deleted player
  PDat(Pt).Won = 0
  ...
  IF nextAction > 0 THEN
   cStat = 0: GOSUB Curs 'Ã¾ Remove cursor bar
  END IF
  ...
  ELSEIF N = PDefs(opp) THEN
   IF N > 1 THEN
    x = x - 1: IF x = 0 THEN y = y - 1: x = 4 'Ã¾ Back one player
   ELSEIF N < P THEN
    x = x + 1: IF x = 5 THEN y = y + 1: x = 1 'Ã¾ Fwd one player
   END IF
  END IF
  ...
  'Ã¾ Ensure always 2 players minimum
...
Move: 'Ã¾ Displays or removes cursor bar, calculating highlight colour
 which = 1
 IF PDefs(2 / fld) = N THEN which = 0
 IF PDefs(fld) = N THEN which = 2
 SELECT CASE cur 'Ã¾ Blue (1) or black (0) background
  CASE 1
   IF which = 1 THEN
    cStat = 1: GOSUB Curs
   ELSEIF which = 0 THEN
...
Curs: 'Ã¾ Displays or removes cursor bar, being told the highlight colour
 SELECT CASE cStat
  CASE 0
   COLOR 15, 0
  CASE 1
...
SUB GorillaIntro (Player$(), cIntro)
  IF cIntro = 1 THEN 'Ã¾ cIntro = 0 means introduction compulsory
   CLS
   LOCATE 1, 36: PRINT STRING$(10, " ")
   RESTORE Ready: Slidy
   COLOR 2: LOCATE 15, 31: PRINT "ÃÄ=ð=Ä=ð=Ä=ð=Ä=ð=Ä=ð=Ä=ð=Ä=ð=Ä´"
...
FUNCTION PlayGame (Player$(), NumGames, P)
 ...
 'Ã¾ If game played out then go through end game sequence
 IF NOT abortYN THEN
  GamePlayedYN = 1
...
FUNCTION PlotShot (StartX, StartY, angle#, velocity, PlayerNum, othX, othY)
 ...
 SunHit = FALSE
 ShotInSun = FALSE
 OnScreen = TRUE 'Ã¾ FALSE if the banana is off side
 PlayerHit = 0
 NeedErase = FALSE
 Bounced = FALSE
 'Ã¾ Set up banana sound effect
 DoooMinVeloc = 40
 pitch! = 9800
 pitchDec! = 100
 pitchDecDec! = (((InitYVel# - DoooMinVeloc) / (200 - DoooMinVeloc)) * 1.2) - .5

 t2b# = 9999 'Ã¾ Used to store the time when the banana is to stop moving
 ...
 'Ã¾ Obtain predicted x-coordinate when banana reaches bottom of screen
 GOSUB PredictBottomOfScreen
 'Ã¾ See if banana will overshoot (direction is +ve for left & -ve for right)
 'Ã¾ MissedDist# is -ve for miss, and +ve for hit
 IF Direction > 0 THEN
  MissedDist# = XPredicted#
 ELSE
  MissedDist# = ScrWidth - XPredicted#
 END IF
 'Ã¾ If shot is going backwards, then turns it into a miss
 ...
 DO WHILE (NOT Impact) AND OnScreen
  ...
  IF y# > oldy# AND InitYVel# > DoooMinVeloc AND NOT Bounced AND MissedDist# > -175 THEN
   'Ã¾ Play banana sound effect
   IF GSettings.useSound THEN SOUND pitch!, 1
   'Ã¾ Decrement banana sound effect pitch
  ...
  IF y# >= ScrHeight - 7 THEN
   'Ã¾ If velocity is still high enough to bounce, and banana is on screen
   IF InitYVel# > 2 AND t2b# = 9999 THEN
    ...
    t# = 0
   ELSE
    'Ã¾ Terminate banana motion
    OnScreen = FALSE
    DoSun SUNHAPPY
    IF t2b# = 9999 THEN 'Ã¾ Because its velocity ran out
     IF GSettings.useSound THEN PLAY "O0A4"
    ELSE 'Ã¾ Or because it bounced when off screen
     IF SGN(Direction) <> SGN(InitXVel#) THEN GOSUB FailureMessage
    END IF
   END IF
  END IF

  'Ã¾ If banana leaves the screen
  IF (x# >= ScrWidth - Scl(10)) OR (x# <= 3) THEN
   'Ã¾ And banana will not return to the screen
   IF (XPredicted# >= ScrWidth - Scl(10)) OR (XPredicted# <= 3) THEN
    IF t# > t2b# THEN
     OnScreen = FALSE
     'Ã¾ Redraw sun as soon as poss
 ...
 ELSEIF pointval <> OBJECTCOLOR AND Impact THEN
  ...
  'Ã¾ Reset values for shot's initial stage (before any bouncing)
  InitXVel# = COS(angle#) * velocity
  InitYVel# = SIN(angle#) * velocity
  StartXPos = StartX: IF PlayerNum = 2 THEN StartXPos = StartXPos + Scl(25)
  StartYPos = StartY - adjust - 3
  GOSUB PredictReturnToHeight
  'Ã¾ If shot went the right direction...
  IF SGN(Direction) <> SGN(InitXVel#) THEN
   'Ã¾ ...and if shot was too low powered:
 ...
 'Ã¾ Message for backwards-tossed shot
...
FailureMessage:
 'Ã¾ Select message based on distance beyond screen edge
 'Ã¾ NOT calibrated for CGA

 GiveDist = 0 'Ã¾ Flag to indicate whether to show distance travelled

 'Ã¾ If the player saw the banana leave the screen
...
pCreatePlayer:
 ...
 'Ã¾ Used if the ESCAPE prompt is to be given
IF fieldNum = 12 THEN LOCATE 6, 3: PRINT ""
 ...
 'Ã¾ Signals whether ESCAPE can be pressed
...
SUB SparklePause (opt AS INTEGER)
 ...
 DO: LOOP UNTIL INKEY$ = "" 'Ã¾ Clear keyboard buffer
 COLOR 12, 0
 a$ = "*    *    *    *    *    *    *    *    *    *    *    *    *    *    *    *    *    "
 t! = TIMER
 DO
...
SUB Stats (Wins(), nam$(), Ban!(), P, abortYN)
 ...
 ELSE
  'Ã¾ Update and sort the league table
  ...
  'Ã¾ routine to sort the player list
  DO
 ...
 'Ã¾ Show league table no matter what
 LOCATE 8, 20: COLOR 9: PRINT "STATISTICS";
 LOCATE 9, 3: COLOR 2: PRINT "É"; STRING$(74, "Í"); "»";
 FOR l = 10 TO 20: LOCATE l, 3: PRINT "º"; TAB(78); "º"; : NEXT
 LOCATE 21, 3: PRINT "È"; STRING$(74, "Í"); "¼";
 COLOR 3
 ...
 'Ã¾ Only save stats if they have changed or if file absent
